...
const { Item } = List;
class MadridForm extends Component{
  constructor(props){
    super(props);
    this.state = {
      enterpriseNatureOp: [
  ...
  onSubmit() {
    this.props.form.validateFields({ force: true }, (error) => {
      if (!error) {
        ...
        const value = { logoUrl, ...this.props.form.getFieldsValue() };
        Object.keys(value).forEach(k => {
          if (value[k] instanceof Array) value[k] = value[k][0]
          if (!value[k]) Toast.info('请完善数据', 1)
        })
        api.post('/0101_apply_sub.htm', value).then(res => {
    ...
    return (
      <div className='goldLibraryDetailForm-component' style={{ background: '#fff', position: 'fixed', zIndex: 200, left: '0', right: '0', top: '0', bottom: '0' }}>
        ...
        <List>
          ...
          <InputItem {...getFieldProps('cityArea')} clear placeholder="">所处市/区:</InputItem>
          ...
          <Item><div>申请商标图样</div>
            <ImagePicker files={this.state.logoUrlFiles} multiple={false} selectable={this.state.logoUrlFiles.length < 1} onChange={(files, type) => { this.onChange(files, type, 'logoUrlFiles') }} />
          </Item>
          <InputItem {...getFieldProps('logoType')} clear placeholder="">商标类别:</InputItem>
        </List>
        </div>
...
export default createForm()(MadridForm);